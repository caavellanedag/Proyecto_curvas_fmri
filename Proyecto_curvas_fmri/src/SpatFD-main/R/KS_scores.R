KS_scores <- function(SFD, newcoords, model, name = NULL, fill.all = NULL){
     ...
     if(fill.all == T){
          ...
          for (i in 1:ncol(puntajes)){
                  ...
                  Variogram_selection <- map_dfr(c("Exp", "Sph", "Gau"), function(.x){
                          
                          v <- variogram(puntajes[[i]] ~ 1, puntajes)
                          fit.vgm <- try(autofitVariogram(puntajes[[i]] ~ 1, puntajes, model = .x), silent = TRUE)
                          
                          if(class(fit.vgm) == "try-error"){
                                data.frame(Model = .x, SSE = 10*10^100)  
                          }else{
                                fv <- fit.variogram(v, 
                                                      vgm(psill = max(fit.vgm$var_model[2, 2], 0.5),
                                                          .x, 
                                                          range = max(fit.vgm$var_model[2, 3], 2),
                                                          nugget = max(fit.vgm$var_model[1, 2], 0), cutoff = 25)
                                                    )
                                fitted_variogram <- try(variogramLine(fv, dist_vector = v$dist), silent = TRUE)
                          if(class(fitted_variogram) == "try-error"){
                                data.frame(Model = .x, SSE = 10*10^100)  
                          }else{
                                
                                data.frame(Model = .x, SSE = sum((fitted_variogram$gamma - v$gamma)^2))
                          }
                          }
                          
                  })
                  Theorical_model <- Variogram_selection$Model[which.min(Variogram_selection$SSE)]
                  fit.vgm <- autofitVariogram(puntajes[[i]] ~ 1, puntajes, model = Theorical_model)
                  v[[i]] <- variogram(puntajes[[i]] ~ 1, puntajes)
                  fv[[i]] <- fit.variogram(v[[i]], vgm(psill = max(fit.vgm$var_model[2, 2], 0.5), 
                                                       Theorical_model,
                                                       range = max(fit.vgm$var_model[2, 3], 2),
                                                       nugget = max(fit.vgm$var_model[1, 2], 0),
                                                       cutoff = 25))
